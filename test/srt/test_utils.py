...
class TestAiterTopKRoutingBuffers(unittest.TestCase):
    def test_initialization(self):
        ...
        for (
            ...
            n_routed_experts,
            n_shared_experts,
            mandatory_shared_expert_ids,
        ) in param_list:
            with self.subTest(
                top_k=top_k,
                n_routed_experts=n_routed_experts,
                n_shared_experts=n_shared_experts,
                mandatory_shared_expert_ids=mandatory_shared_expert_ids,
            ):
                ...
                self.assertEqual(
                    buffers.non_shared_topk_ids.shape,
                ...
                self.assertEqual(
                    first_token_non_shared_experts_ids,
                    [0] * top_k,
                    "Non shared expert ids should be empty",
                )

                first_token_shared_experts_ids = first_token_expert_ids[-top_k:]
                self.assertEqual(
                    first_token_shared_experts_ids,
                    mandatory_shared_expert_ids,
                    "Each token has to be also routed to all shared experts",
                )
                ...
                self.assertEqual(
                    buffers.non_shared_topk_weights.shape,
                    (AiterTopKRoutingBuffers.MAX_NUM_TOKENS, top_k),
                )

                first_token_shared_weights = buffers.total_topk_weights[0].tolist()[
                    -top_k:
                ]
                self.assertTrue(
                    all(
                        weight == AiterTopKRoutingBuffers.SHARED_EXPERTS_SCORE
                        for weight in first_token_shared_weights
                    ),
