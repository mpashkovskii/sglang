...
class TestFp8MoEBlockscaleBuffers(unittest.TestCase):
    def test_initialization(self):
        ...
        for top_k, n_routed_experts, n_shared_experts, mandatory_shared_expert_ids in param_list:
            with self.subTest(top_k=top_k, n_routed_experts=n_routed_experts, n_shared_experts=n_shared_experts, mandatory_shared_expert_ids=mandatory_shared_expert_ids):
                ...
                buffers = Fp8MoEBlockscaleBuffers(
                    top_k=top_k,
                ...
                self.assertEqual(buffers.non_shared_topk_ids.shape, (Fp8MoEBlockscaleBuffers.AITER_MOE_MAX_NUM_TOKENS, top_k))
                ...
                self.assertEqual(buffers.non_shared_topk_weights.shape, (Fp8MoEBlockscaleBuffers.AITER_MOE_MAX_NUM_TOKENS, top_k))

                first_token_shared_weights = buffers.total_topk_weights[0].tolist()[-top_k:]
                self.assertTrue(
                    all(weight == Fp8MoEBlockscaleBuffers.SHARED_EXPERTS_SCORE for weight in first_token_shared_weights),
